...
function start(opts, cb) {
  ...
  if (process.env.http_proxy) {
    var host = process.env.http_proxy.host.split(":")[0];
    var port = process.env.http_proxy.host.split(":")[1];
    args.push('-Dhttp.proxyHost=' + host);
    if (port) {
      args.push('-Dhttp.proxyPort=' + port);
    }
    args.push('-Dhttp.proxyUser=' + process.env.http_proxy.user);
    args.push('-Dhttp.proxyPassword=' + process.env.http_proxy.passwd);
  }
