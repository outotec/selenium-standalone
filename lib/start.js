...
function start(opts, cb) {
  ...
  if (process.env.http_proxy) {
    // Assumes that the http_proxy is in format "http://user:password@host:port/"
    var re = /[\s\/:@]+/;
    var splittedProxy = process.env.http_proxy.split(re);

    args.push('-Dhttp.proxyUser=' + splittedProxy[1]);
    args.push('-Dhttp.proxyPassword=' + splittedProxy[2]);
    args.push('-Dhttp.proxyHost=' + splittedProxy[3]);
    args.push('-Dhttp.proxyPort=' + splittedProxy[4]);
  }

  args = args.concat(opts.seleniumArgs);

  logger("checkPathsExistence(" + getInstallPaths(fsPaths) + ")");
  checkPathsExistence(getInstallPaths(fsPaths), function(err) {
    ...
    logger("spawning selenium");
    var selenium = spawn(opts.javaPath, args, opts.spawnOptions);
    logger("forward selenium to spawnCb");
    opts.spawnCb(selenium);

    selenium.on('exit', errorIfNeverStarted);

    logger("checkStarted()");
    checkStarted(args, logger, function started(err) {
      logger("callback for checkStarted()");
    ...
    function errorIfNeverStarted(code) {
      ...
      logger("errorIfNeverStarted(" + code + ")");
      neverStarted = true;
