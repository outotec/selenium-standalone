...
exports.getSeleniumStatusUrl = function(seleniumArgs, logger) {
  logger("getSeleniumStatusUrl()");
  var processType = this.getRunningProcessType(seleniumArgs);
  logger("processType: " + processType);
  ...
  switch (processType) {
    ...
    case PROCESS_TYPES.GRID_NODE:
      ...
      break;
    default:
      logger('ERROR: Trying to run selenium in an unknown way.');
